...
- **强大的注入点支持** - HEAD, TAIL, RETURN, INVOKE, FIELD, FIELD_ASSIGN, LOAD, STORE, NEW, CAST, THROW，以及 `@ModifyExpressionValue` 使用的 INSTANCEOF，满足各种场景需求
...
- **精确定位能力** - 普通 `@AsmInject(INVOKE/FIELD/FIELD_ASSIGN/LOAD/STORE/CAST/THROW)`、`@Redirect`、参数/receiver/表达式/变量/常量修改与操作包裹可用 `Slice` 把候选点限制在指定调用边界内；普通 `@AsmInject(LOAD/STORE)` 还可按局部变量槽位过滤
