...
- **强大的注入点支持** - HEAD, TAIL, RETURN, INVOKE, INVOKE_ASSIGN, INVOKE_STRING, FIELD, FIELD_ASSIGN, LOAD, STORE, NEW, CAST, INSTANCEOF, JUMP, SWITCH, CONSTANT, THROW，以及裸数组长度使用的 ARRAY_LENGTH，满足各种场景需求；普通 `@AsmInject` 不支持 `ARRAY_LENGTH`，裸 `ARRAYLENGTH` 请使用 `@Redirect`、`@WrapOperation`、`@WrapWithCondition` 或 `@ModifyExpressionValue`
