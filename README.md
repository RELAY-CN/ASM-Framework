...
- **强大的注入点支持** - HEAD, TAIL, RETURN, INVOKE, FIELD, FIELD_ASSIGN, NEW, CAST, THROW，以及 `@ModifyVariable` 使用的 LOAD/STORE，满足各种场景需求
...
- **表达式改写能力** - `@ModifyExpressionValue` 可改写调用返回值、字段读取值、数组元素读取值、数组长度、对象构造结果或类型转换结果
