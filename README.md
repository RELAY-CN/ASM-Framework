...
- **注入命中数契约** - `@AsmInject`、`@ModifyArg`、`@ModifyArgs`、`@ModifyReceiver`、`@WrapOperation`、`@ModifyVariable`、`@ModifyReturnValue`、`@ModifyExpressionValue`、`@ModifyConstant` 与 `@Redirect` 支持 `require`、`allow` 与 `expect`，可在目标字节码漂移时快速发现补丁失效
