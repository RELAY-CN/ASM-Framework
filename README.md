...
- **实用的重定向能力** - `@Redirect` 可替换方法调用、`invokedynamic` 调用、构造器调用、字段读取、字段写入、简单数组元素读写、数组长度读取、类型转换与类型判断，并支持追加目标方法参数前缀；`@RedirectAllMethods` 可把同一组重定向规则应用到目标类所有普通方法，并按全类总命中数校验 `require` / `allow` / `expect`
