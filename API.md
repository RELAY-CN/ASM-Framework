...
- `slice: Slice = Slice()` - 切片范围；当前 `LOAD` 模式支持用 `INVOKE` 边界缩小查找范围
- `remap: Boolean = false` - 是否重映射

`@ModifyVariable` 的 `LOAD` 模式可用 `slice.from` / `slice.to` 把候选 `xLOAD` 读取点限制在一段 `INVOKE` 边界之间；边界调用本身不参与候选匹配，`ordinal` 会在切片内重新计数。`HEAD` 与 `STORE` 当前不使用 `slice`。
...
形式的常量加载。ASM 方法的第一个参数必须接收原始常量，并返回同类型的新值；后续参数可以按目标方法声明顺序接收目标方法参数前缀。未指定 `constant` 时会按 handler 返回类型筛选常量，再用 `ordinal` 选择第 N 个候选。
...
`@ModifyVariable(at.value = InjectionPoint.LOAD)`
