...
它不作为普通扩展 API 暴露；旧版方法元信息值对象、监听器与重定向替换兼容层已移除。迁移说明见 [REDIRECTION_MIGRATION.md](REDIRECTION_MIGRATION.md)。
...
接口名会去除首尾空白，空白接口名会在转换阶段失败；已存在的接口会被跳过，不会重复写入。
...
该注解只修改目标类的 `interfaces` 列表，不会删除目标类中已有的方法实现。接口名会去除首尾空白，空白接口名会在转换阶段失败；目标类未声明的接口会被跳过，不视为转换失败。
...
普通 `INVOKE_ASSIGN` 可在调用后观察位置；如果原调用必须执行但返回表达式只在条件满足时采纳，应使用 `@WrapWithCondition(at = At(value = InjectionPoint.INVOKE_ASSIGN, target = "..."))`。

普通 `CAST` 只观察类型转换位置；只需要按条件决定是否保留 `CHECKCAST` 转换结果时使用 `@WrapWithCondition(at = At(value = InjectionPoint.CAST, target = "..."))`。
...
- `at: At = At()` - 注入位置；默认 `HEAD` 改写入口参数，`at.value = InjectionPoint.INVOKE` 时用 `at.target` 匹配目标方法调用、构造器调用或 `invokedynamic` 调用；`at.target` 为空则按 handler 签名推断兼容调用点；显式 `at.target` 可写 `owner.name(desc)`、`owner/name(desc)` 或 `name(desc)`，但必须包含方法描述符，缺失时会在转换阶段失败；可用唯一的 `at.args = ["ldc=value"]` 或 `["string=value"]` 只匹配调用参数直接来自该字符串常量的调用点
...
- `at: At = At(value = InjectionPoint.INVOKE)` - 调用点定位；当前仅支持 `INVOKE`，可匹配普通方法调用、构造器调用或 `invokedynamic` 调用；`at.target` 为空时按兼容调用点推断；显式 `at.target` 可写 `owner.name(desc)`、`owner/name(desc)` 或 `name(desc)`，但必须包含方法描述符，缺失时会在转换阶段失败；可用唯一的 `at.args = ["ldc=value"]` 或 `["string=value"]` 只匹配调用参数直接来自该字符串常量的调用点
...
显式 `FIELD` / `FIELD_ASSIGN` 字段目标可写 `owner.field:desc`、`field:desc` 或 `field`，字段名必填；只有省略整个 `At.target` 才进入兼容候选推断。
...
字段来源的数组长度读取使用 `FIELD + at.args = ["array=length"]`，`At.target` 指向产生数组引用的数组字段。
非字段来源的裸 `ARRAYLENGTH` 使用 `ARRAY_LENGTH`，不使用 `At.target`。handler 先接收 `Int` 长度值，
不接收数组引用，后续可继续接收目标方法参数前缀。
...
局部变量写入、类型转换、类型判断、常量加载、条件跳转、switch selector 与抛异常点均按实际条件包裹数量计数；省略 `INVOKE` 调用目标、
`FIELD` 字段目标、`FIELD_ASSIGN` 字段目标、`CAST` 类型目标、`INSTANCEOF` 类型目标、`CONSTANT` 常量目标或 `THROW` 异常类型目标时，
不兼容候选不计入数量；省略 `JUMP` 跳转目标时会按兼容 handler 匹配全部条件跳转；`SWITCH` 不支持 `At.target`；
...
- `slice: Slice = Slice()` - 切片范围；当前支持用 `INVOKE`、`FIELD`、`FIELD_ASSIGN` 或 `CONSTANT` 边界缩小常量查找范围；声明这些边界时 `At.target` 必填，字段边界还必须包含字段名，默认 `At()` 才表示未声明边界
...
会替换匹配的 `PUTFIELD` / `PUTSTATIC` 指令。数组元素访问与字段来源数组长度重定向使用 `at.value = InjectionPoint.FIELD` 匹配产生数组引用的字段，
并通过 `at.args = ["array=get"]` 或 `at.args = ["array=length"]` 区分数组读取与长度读取；数组元素写入使用
`at.value = InjectionPoint.FIELD_ASSIGN` 与 `at.args = ["array=set"]`。
显式 `FIELD` / `FIELD_ASSIGN` 字段目标可写 `owner.field:desc`、`field:desc` 或 `field`，字段名必填；只有省略整个 `At.target` 才进入兼容候选推断。
非字段来源的裸 `ARRAYLENGTH` 使用 `at.value = InjectionPoint.ARRAY_LENGTH`，不使用 `At.target` 或 `At.args`；字段来源数组长度继续使用 `FIELD + at.args = ["array=length"]`。
handler 首参按 `Any` / `Object` 接收被 `ARRAYLENGTH` 消费的数组引用，并返回替代长度 `Int`。
...
条件跳转重定向使用 `at.value = InjectionPoint.JUMP` 与跳转操作码名或数字匹配条件跳转指令，handler 接收原始分支结果 `Boolean` 并返回新的 `Boolean` 分支结果。省略 `at.target` 时会匹配切片内全部条件跳转，`GOTO` 与 `JSR` 不支持重定向。
...
方法调用、`invokedynamic` 调用、构造器调用、NEW 构造表达式、字段读取、字段写入、数组元素访问、字段来源或裸数组长度、局部变量读取、局部变量待写入值、类型转换、类型判断、条件跳转、switch selector、常量加载与抛异常点重定向的 handler 可以是静态方法、`@JvmStatic` 方法，或 Kotlin `object` 中的实例方法。
...
- 数组长度读取：字段来源模式接收数组引用，并可继续追加目标方法参数前缀，返回 `Int`；裸 `ARRAY_LENGTH` 模式同样接收数组引用，但参数契约固定为 `Any` / `Object`
...
方法调用、`invokedynamic` 调用、构造器调用、NEW 构造表达式、字段读取、字段写入、数组元素访问、字段来源或裸数组长度、局部变量读取、局部变量待写入值、类型转换、类型判断、条件跳转、switch selector、常量加载与抛异常点重定向都可用 `ordinal` 只替换第 N 个匹配点。
...
框架会从 `getXxx`、`setXxx` 或 `isXxx` 方法名推断字段名；若后缀前两个字符均为大写，会保留原后缀，
例如 `getURL`、`setURL` 或 `isURL` 会推断为 `URL`，而不是 `uRL`。目标字段不存在、
...
若后缀前两个字符均为大写，会保留原后缀，例如 `callURL` 或 `invokeURL` 会推断为 `URL`；
...
##### `setTypedReturnValue(value: T?)`

设置带类级泛型标注的新返回值。该方法等价于 `setReturnValue(value)`，但参数类型为 `T?`，适合 Kotlin handler 保持返回值类型提示。

##### `getReturnValueZ/B/C/D/F/I/J/S()`

按 JVM 基础类型后缀读取返回值，命名兼容 Mixin 的 typed getter 习惯。当前值不是对应类型时返回该基础类型默认值；数值类型会按目标基础类型转换。
...
- `INVOKE` - 方法调用前
- `INVOKE_ASSIGN` - 方法调用后
- `INVOKE_STRING` - 带直接字符串常量实参的方法调用前
- `FIELD` - 字段访问前
- `FIELD_ASSIGN` - 字段赋值前
- `LOAD` - 局部变量读取前
- `STORE` - 局部变量写入后
...
普通 `@AsmInject(INVOKE_STRING)` 使用指令点注入器，只观察普通方法调用实参中的直接 `LDC String`，必须通过 `At.target` 指定普通方法调用，并通过 `args = ["ldc=value"]` 或 `["string=value"]` 指定字符串常量；它不会接收或替换该字符串，也不会匹配局部变量、字符串拼接、方法返回值或 `invokedynamic`。`@ModifyArg(INVOKE)`、`@ModifyArgs(INVOKE)`、`@Redirect(INVOKE)`、`@WrapOperation(INVOKE)`、`@WrapWithCondition(INVOKE/INVOKE_ASSIGN)` 与 `@ModifyExpressionValue(INVOKE/INVOKE_ASSIGN)` 可用同样的 `args` 过滤调用参数直接来自指定字符串常量的候选，过滤后再计算 `ordinal` 与命中数。
...
`CHECKCAST` 完成后的对象表达式值”，把 `INSTANCEOF` 解释为“匹配类型判断后的 boolean 结果”，把 `LOAD` 解释为“匹配局部变量读取表达式值”，把 `STORE` 解释为“匹配局部变量写入前的待写入表达式值”，把 `JUMP` 解释为“匹配条件跳转的 boolean 分支结果”，把 `SWITCH` 解释为“匹配 `tableswitch` / `lookupswitch` 消费前的 `Int` selector”。`@ModifyReceiver(INVOKE)` 可用 `At.args` 中唯一的 `ldc=<string>` 或 `string=<string>` 按调用参数直接字符串来源过滤候选，但不会匹配 receiver 本身。`@ModifyReceiver` 会把 `INVOKE`
...
当 `FIELD + args = ["array=length"]` 时解释为“匹配字段来源数组长度读取结果后的条件判断”，把 `FIELD_ASSIGN`
解释为“匹配字段写入或数组元素写入前的条件判断”，把 `ARRAY_LENGTH` 解释为“匹配裸 `ARRAYLENGTH` 结果后的条件判断”，把 `INVOKE_ASSIGN` 解释为“匹配调用完成后的返回值条件判断”，把 `LOAD` 解释为“匹配局部变量读取结果后的条件判断”，把 `STORE` 解释为“匹配局部变量写入前的条件判断”，把 `NEW` 解释为“匹配构造完成后的对象表达式条件判断”，把 `CAST` 解释为“匹配类型转换结果后的条件判断”，把 `INSTANCEOF` 解释为“匹配类型判断结果后的条件判断”，把 `CONSTANT` 解释为“匹配常量加载结果后的条件判断”，把 `JUMP` 解释为“匹配条件跳转分支结果前的条件判断”，把 `SWITCH` 解释为“匹配 switch selector 前的条件判断”，把 `THROW` 解释为“匹配即将抛出的异常前的条件判断”。普通 `@AsmInject(INVOKE_STRING/FIELD/FIELD_ASSIGN/LOAD/STORE/CAST/INSTANCEOF/JUMP/SWITCH/CONSTANT/THROW/ARRAY_LENGTH)`
...
`@ModifyExpressionValue(INVOKE/INVOKE_ASSIGN)` 使用同样格式过滤直接字符串实参调用点，但分别继续替换选中参数、参数组、调用本身、操作包裹、条件包裹或调用返回表达式。普通 `@AsmInject(LOAD/STORE)` 只作为局部变量读写指令附近的观察 hook，
...
- `args: Array<String> = []` - 附加定位参数；`@Redirect` 当前支持 `array=get`、`array=set`、
  ...
  与 `array=length`，以及 `LOAD` / `STORE` 的 `index=N`、`var=N` 与 `name=localName` 局部变量过滤，其中 `array=get` / `array=length` 需配合 `FIELD`，`array=set` 需配合 `FIELD_ASSIGN`，`ARRAY_LENGTH` 不需要附加参数；普通 `@AsmInject(ARRAY_LENGTH)` 不使用附加参数；普通 `@AsmInject(INVOKE_STRING)` 当前必须且只能声明一个 `ldc=<string>` 或 `string=<string>` 直接字符串常量实参过滤；`@ModifyArg(INVOKE)`、`@ModifyArgs(INVOKE)`、`@Redirect(INVOKE)`、`@WrapOperation(INVOKE)`、`@WrapWithCondition(INVOKE/INVOKE_ASSIGN)` 与 `@ModifyExpressionValue(INVOKE/INVOKE_ASSIGN)` 可选声明同样格式的直接字符串实参过滤；普通 `@AsmInject(LOAD/STORE)` 当前支持 `index=N`、`var=N` 与 `name=localName`

**`target` 格式：**

- `INVOKE`: `owner.name(desc)`、`owner/name(desc)` 或 `name(desc)`，owner 可使用 JVM internal name 或 Java binary name，例如 `java/lang/String.trim()Ljava/lang/String;` 或 `java.lang.String.trim()Ljava/lang/String;`
- `INVOKE_STRING`: `owner.name(desc)` 或 `owner/name(desc)`，并通过唯一一个 `args = ["ldc=value"]` 或 `["string=value"]` 匹配调用实参中的直接字符串常量；只支持普通方法调用，owner 必填
...
- `JUMP`: 跳转操作码名或数字操作码，例如 `IFEQ`、`IF_ICMPGT` 或 `153`；普通 `@AsmInject(JUMP)` 省略时匹配所有跳转，`@Redirect(JUMP)`、`@ModifyExpressionValue(JUMP)`、`@WrapOperation(JUMP)` 与 `@WrapWithCondition(JUMP)` 省略时只匹配切片内全部条件跳转
...
- `ARRAY_LENGTH`: 不使用 `target` 或 `args`；普通 `@AsmInject(ARRAY_LENGTH)`、`@Redirect(ARRAY_LENGTH)`、`@WrapOperation(ARRAY_LENGTH)`、`@WrapWithCondition(ARRAY_LENGTH)` 与 `@ModifyExpressionValue(ARRAY_LENGTH)` 直接匹配裸 `ARRAYLENGTH` 指令

`@Redirect` 可在 `FIELD` 目标上使用 `args = ["array=get"]` 或 `args = ["array=length"]`，在 `FIELD_ASSIGN` 目标上使用 `args = ["array=set"]`，
把目标字段解释为产生数组引用的字段，并重定向紧随其后的数组元素读取、`ARRAYLENGTH` 或数组元素写入。若不需要限定字段来源，`@Redirect(ARRAY_LENGTH)` 可直接匹配裸 `ARRAYLENGTH`，不使用 `At.target` 或 `At.args`，handler 接收 `Any` / `Object` 数组引用并返回 `Int`。`@Redirect(LOAD)` 可用 `args = ["index=N"]`、`["var=N"]` 或 `["name=localName"]` 重定向指定 JVM 局部变量槽位或 LocalVariableTable 变量名的本次读取值，只替换这一次读取结果，不写回槽位；`@Redirect(STORE)` 可用同样的槽位或名称过滤改写 `xSTORE` 消费前的待写入栈顶值，返回值交给原 `xSTORE` 继续写入槽位。`@WrapOperation`
...
普通 `@AsmInject(ARRAY_LENGTH)` 可观察裸 `ARRAYLENGTH` 指令，不接收数组引用或长度值，不替换长度结果，并且不使用 `At.target` 或 `At.args`；字段来源数组长度仍使用 `FIELD + args = ["array=length"]`。需要读取或改写裸数组长度时，应使用 `@Redirect`、`@WrapOperation`、`@WrapWithCondition` 或 `@ModifyExpressionValue`。
...
`@ModifyArg(INVOKE)`、`@ModifyArgs(INVOKE)`、`@ModifyReceiver(INVOKE)`、`@Redirect(INVOKE)`、`@WrapOperation(INVOKE)`、`@WrapWithCondition(INVOKE/INVOKE_ASSIGN)` 与 `@ModifyExpressionValue(INVOKE/INVOKE_ASSIGN)` 的直接字符串实参过滤同样只检查调用参数来源，不匹配 receiver；过滤值按 `ldc=` / `string=` 后的完整文本精确匹配，前后空格不会被修剪；配置多个 `args` 或未知前缀会在转换阶段失败。
