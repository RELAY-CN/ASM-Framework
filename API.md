...
`@ModifyReturnValue` handler 返回类型对 primitive 必须与目标方法返回类型一致；对象/数组返回值可以是目标类型的子类型，也可以用 `Any` / `Object` 作为泛型引用返回类型，框架会在 handler 调用后转换回目标返回类型。参数可选：第一个参数可接收原始返回值，后续参数可按目标方法声明顺序接收目标方法参数前缀；当原返回值或追加的目标方法参数是对象/数组类型时，对应 handler 参数可声明为原值类型的父类、接口、`Any` 或 `Object`。`method` 为空时会先按 handler 名称筛选目标方法，再按返回类型、原返回值参数和目标方法参数前缀的兼容规则匹配唯一同名目标；如果多个重载都兼容，需要显式写出 `method` 签名。
...
目标类自身没有该字段时，访问器会沿可加载父类查找可继承字段，并使用字段实际 owner 生成访问指令；
静态继承字段仍要求访问器方法也是静态方法。
...
目标方法会先在目标类自身查找，找不到时沿可加载父类查找非 `private` 继承方法；目标方法不存在、
参数/返回值不匹配、静态性不匹配，或生成方法与目标类已有方法冲突时，转换会失败。
目标为接口私有方法时，桥接调用会使用 JVM 要求的 `INVOKESPECIAL`；普通接口方法仍使用
`INVOKEINTERFACE`。
