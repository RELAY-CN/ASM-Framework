...
- **@WrapOperation** - 用可调用原操作的 handler 包裹方法调用、`invokedynamic` 调用、构造器调用、字段读取、字段写入、数组元素读写、字段来源或裸 `ARRAYLENGTH` 数组长度读取、类型转换、类型判断、局部变量读取或待写入值、条件跳转、switch selector、常量读取或即将抛出的异常；`INVOKE` 可按直接字符串实参过滤
- **@WrapMethod** - 用可调用原方法的 handler 包裹整个目标方法体
- **@WrapWithCondition** - 按条件跳过普通调用、`invokedynamic` 调用，或在调用完成后条件保留返回值，也可条件保留字段读取、字段写入、数组元素读取、数组元素写入、字段来源或裸 `ARRAYLENGTH` 数组长度读取、对象构造结果、局部变量读取或写入、类型转换、类型判断、常量加载、条件跳转、switch selector 或即将抛出的异常；`INVOKE` / `INVOKE_ASSIGN` 可按直接字符串实参过滤
- **@ModifyExpressionValue** - 修改表达式值；`INVOKE` / `INVOKE_ASSIGN` 可按直接字符串实参过滤
...
`CallbackInfoReturnable<T>`。`HEAD`、`TAIL`、`RETURN` 与字段、`NEW`、`CAST`、`INSTANCEOF`、`JUMP`、`SWITCH`、`CONSTANT`、`ARRAY_LENGTH`、`THROW`
...
只有声明 `cancellable = true` 的可取消回调才能调用 `CallbackInfo.cancel()`；当前 `HEAD`、`TAIL` 与普通 `INVOKE` / `INVOKE_ASSIGN`
的 `BEFORE` / `AFTER` 注入会在取消后提前返回。
如果目标方法有返回值，可取消回调调用 `CallbackInfo.setReturnValue(...)` 也会自动标记取消并返回该值。
普通 `RETURN` 与非 `void` `TAIL` 注入会把原始返回值预置到 `CallbackInfo`，handler 可调用 `setReturnValue(null)` 把引用类型返回值明确替换为 `null`；
`TAIL` handler 修改后的回调返回值会写回原返回点。
...
需要在普通 `@AsmInject` 中只读观察局部变量时，可以在 `TAIL` / `RETURN` 或普通指令点 handler 参数上显式标记 `@Local`：
...
普通指令点也可以只读捕获同一作用域内的局部变量，而不接收或替换该指令本身的栈顶值：
...
@JvmStatic
fun beforeReadSecond(
    @Local(name = "first") first: String,
    input: String,
) {
    println("input=$input first=$first")
}
```

`@Local` 适合日志、诊断和取消判断这类只读观察局部变量的场景。它读取当前注入锚点可见的
LocalVariableTable 局部变量，缺少调试变量表、缺少匹配名称或槽位类型不兼容时会在转换阶段失败；变量名不稳定但
LocalVariableTable 仍保留时，可改用 `@Local(index = N)` 按槽位过滤。`INVOKE` / `INVOKE_ASSIGN` 注入仍按调用点参数协议传参。
如果需要写回局部变量，应使用
...
`from` 边界之后、`to` 边界之前的调用点、带直接字符串常量实参的调用点、字段读写指令、局部变量读写指令、对象创建、类型转换指令、跳转指令、switch 指令、常量加载、裸数组长度或抛异常指令才会参与匹配，边界本身不会被注入；
`ordinal` 会在切片内重新计数。
...
普通 `@AsmInject(SWITCH)` 只在 `tableswitch` 或 `lookupswitch` 指令前后插入 handler，不接收也不改写 selector，且不支持 `At.target`。需要直接替换 switch selector 时可使用 `@Redirect(SWITCH)`；需要保留原指令并改写 selector 时可使用 `@ModifyExpressionValue(SWITCH)`；需要保留可调用原 selector 的操作句柄时可使用 `@WrapOperation(SWITCH)`；只需要按额外条件决定是否保留原 selector 时可使用 `@WrapWithCondition(SWITCH)`。
...
object ValidationMixin {
    ...
    fun rewriteLiteralConcatArgument(input: String): String = input.replace("unsafe", "safe")
    ...
    fun rewriteLegacyJoinArguments(args: Args) {
        args.set(0, "modern")
    ...
    fun rewriteJoinArguments(args: Args, prefix: String, count: Int) {
        ...
        args.setAll("$prefix:${joined.substringBefore('|').trim()}", "normalized", count)
    }
    ...
    fun rewriteCompatibleReceiver(receiver: String): String = receiver.trim()
    ...
    fun decorateCompatibleCall(
        receiver: String,
        value: String,
        operation: Operation<String>,
    ): String = operation.call(receiver, "compatible-$value")
    ...
    @WrapOperation(
        ...
        at = At(value = InjectionPoint.CONSTANT, target = "vanilla"),
    )
    fun wrapLabelConstant(
        value: String,
        operation: Operation<String>,
    ...
    @WrapOperation(
        method = "chooseRoute(I)Ljava/lang/String;",
        at = At(value = InjectionPoint.SWITCH),
    )
    fun wrapSwitchSelector(
        selector: Int,
        operation: Operation<Int>,
        requested: Int,
    ): Int =
        if (requested < 0) 0 else operation.call(selector)

    @WrapOperation(
        method = "failRoute(Ljava/lang/String;)V",
        at = At(value = InjectionPoint.THROW, target = "java.lang.IllegalStateException"),
    )
    fun wrapRouteFailure(
        throwable: Throwable,
        operation: Operation<Throwable>,
        route: String,
    ): Throwable = IllegalArgumentException("$route:${operation.call(throwable).message}")
    ...
    fun shouldRunCompatibleVoidCall(message: String): Boolean =
        message.isNotBlank()
    ...
    @WrapWithCondition(
        method = "readRoute(I)Ljava/lang/String;",
        at = At(
            value = InjectionPoint.FIELD,
            target = "com/example/Player.routes:[Ljava/lang/String;",
            args = ["array=get"],
    ...
    fun shouldUseRoute(route: String, routeIndex: Int): Boolean =
        routeIndex >= 0 && route.isNotBlank()
    ...
    fun shouldKeepBuilder(builder: StringBuilder, name: String): Boolean =
        name.isNotBlank() && builder.capacity() >= 0
    ...
    fun shouldUseCastValue(value: String, raw: Any): Boolean =
        value.isNotBlank() && value === raw
    ...
    fun rewriteCompatibleStringCall(value: String, prefix: String, count: Int): String =
    ...
    @ModifyExpressionValue(
        method = "storeDisplayName(Ljava/lang/String;)V",
        at = At(
            value = InjectionPoint.FIELD_ASSIGN,
            target = "com/example/Player.name:Ljava/lang/String;",
        ),
    )
    fun rewriteAssignedFieldValue(value: String): String = value.trim()
    ...
    fun rewriteLocalRouteCount(value: Int, bonus: Int): Int = value + bonus

    @ModifyExpressionValue(
        method = "writeName(ILjava/lang/String;)V",
        at = At(
            value = InjectionPoint.FIELD_ASSIGN,
            target = "com/example/Player.names:[Ljava/lang/String;",
            args = ["array=set"],
        ),
    )
    fun rewriteArrayWriteValue(value: String): String = value.trim()
    ...
    fun rewriteSwitchSelector(value: Int, requested: Int): Int =
        if (requested < 0) 0 else value
    ...
    fun rewriteLoadedArgumentOnly(value: String): String = value.trim()
...
`@ModifyArg` 默认使用目标方法入口参数索引；入口模式下 `index < 0` 时会按 handler 首参、返回类型和后续目标方法参数前缀推断唯一兼容参数，多个参数都兼容时需要显式写出 `index`。省略 `method` 时，入口参数模式会按 handler 名称、`index` 或推断出的目标参数、handler 首参、返回类型和后续目标方法参数前缀推断唯一同名目标方法，调用点模式会按实际兼容调用点推断唯一同名目标方法；若有多个兼容重载，应显式写出 `method`。当 `at.value = InjectionPoint.INVOKE` 时，会用 `at.target` 匹配目标方法调用、构造器调用或 `invokedynamic` 调用，并把 `index` 解释为目标调用的参数索引；调用点模式下 `index < 0` 时也会按 handler 首参、返回类型和目标方法参数前缀推断唯一兼容调用参数，多个调用参数都兼容时需要显式写出 `index`。省略 `at.target` 时，会按 `index` 指向或推断出的调用参数类型、handler 首参、返回类型和后续目标方法参数前缀筛选兼容调用点，不兼容候选不计入 `ordinal` 或命中数。`at.args` 可声明唯一的 `ldc=<string>` 或 `string=<string>`，只匹配调用参数直接来自该 `LDC String` 的候选；局部变量、字符串拼接、方法返回值和 receiver 来源不会命中，过滤后再计算 `ordinal` 与命中数。handler 第一个参数接收被修改的原参数并返回同类型的新值；对象或数组参数可声明为原值类型的父类、接口、`Any` 或 `Object` 接收，返回值对引用类型可为原参数类型的可赋值子类型，也可用 `Any` 或 `Object` 作为泛型引用返回类型，框架会在 handler 调用后转换回原参数类型，基础类型仍需精确匹配。构造器调用使用 `<init>` 目标，只能修改构造器描述符内的参数，不暴露未初始化 receiver；`invokedynamic` 调用没有 receiver，目标按 bootstrap owner、动态调用名或 bootstrap 名，以及动态调用点描述符匹配；后续参数可继续接收目标方法参数前缀；调用点模式可用 `ordinal` 只选择第 N 个匹配调用点，也可用 `Slice` 限制匹配范围。关键参数补丁可设置 `require` / `allow` 约束实际修改数量，目标字节码漂移时会在转换阶段失败；`expect` 适合调试期记录期望命中数，不一致时只输出警告。`@ModifyArgs` 用于同一个方法调用、构造器调用或 `invokedynamic` 调用需要同时改写多个参数的场景，handler 第一个参数为 `Args`，可通过 `args.get<T>(index)` 或 `args[index]` 读取调用参数，通过 `args.set(index, value)` 或 `args[index] = value` 写回兼容类型的新值，也可用 `args.setAll(...)` 在数量匹配时一次替换整组参数；`args.setAll(...)` 数量不匹配会抛出 `IllegalArgumentException` 且不会部分写入。`args.size` 与 `args.size()` 可读取参数数量，`Args` 实现了 `Iterable<Any?>`，因此支持 `for (value in args)`、`joinToString` 与 `map` 等标准集合扩展。省略 `method` 时会按 handler 名称、`at.target` 调用点、handler 签名和实际兼容调用点推断唯一同名目标方法，省略 `at.target` 时会扫描普通方法调用、构造器调用或 `invokedynamic` 调用，可用 `ordinal`、`Slice`、`require` / `allow`、直接字符串实参过滤或显式目标收窄。`ModifyArgs` 同样支持唯一的 `at.args = ["ldc=<string>"]` 或 `["string=<string>"]` 直接字符串实参过滤，不匹配 receiver、局部变量、拼接字符串或方法返回值。构造器调用使用 `<init>` 目标，`Args` 只包含构造器参数，不包含未初始化 receiver；`invokedynamic` 调用没有 receiver，按 bootstrap owner、动态调用名或 bootstrap 名，以及动态调用点描述符匹配；对象或数组参数可声明为原值类型的父类、接口、`Any` 或 `Object`；后续参数同样可接收目标方法参数前缀，也支持用 `Slice` 限制匹配范围。关键参数组补丁同样可设置 `require` / `allow` / `expect`，按实际改写的调用点数量校验命中契约。`from` 边界之后、`to` 边界之前的调用才会参与匹配，边界本身不会被修改，`ordinal` 会在切片内重新计数。`@ModifyReceiver` 用于只替换实例方法调用、实例字段读取或实例字段写入的 receiver，handler 第一个参数接收原 receiver；省略 `method` 时会按 handler 名称、receiver 操作点、handler 签名和实际兼容候选推断唯一同名目标方法，省略 `INVOKE`、`FIELD` 或 `FIELD_ASSIGN` 目标时也会按兼容候选参与推断。省略 `INVOKE` 调用目标时，会按 handler 首参与返回类型筛选兼容的实例调用 receiver，静态调用、构造器调用和不兼容实例调用不计入 `ordinal` 或命中数；`INVOKE` 可在 `At.args` 中声明唯一的 `ldc=<string>` 或 `string=<string>` 直接字符串实参过滤，只检查调用参数来源，不匹配 receiver，过滤后再计算 `ordinal` 与命中数；字段 receiver 模式不消费 `At.args`，携带会在转换阶段失败；省略 `FIELD` 字段目标时，会按 handler 首参与返回类型筛选兼容的实例字段读取 receiver，静态字段和不兼容字段读取不计入 `ordinal` 或命中数；省略 `FIELD_ASSIGN` 字段目标时，会按 handler 首参与返回类型筛选兼容的实例字段写入 receiver，静态字段和不兼容字段写入不计入 `ordinal` 或命中数。对象或数组 receiver 可声明为原 receiver 类型的父类、接口、`Any` 或 `Object`，并返回兼容的新 receiver，框架会在 handler 调用后把结果转换回原 receiver 类型。`INVOKE` 会保留原调用参数，`FIELD` 会继续读取新 receiver 上的字段，`FIELD_ASSIGN` 会把原待写入值写到新 receiver，三种模式都可用 `Slice` 限制 receiver 改写范围，后续参数可接收目标方法参数前缀。关键 receiver 补丁同样可设置 `require` / `allow` / `expect`，按实际改写的 receiver 数量校验命中契约；设置 `ordinal` 时最多命中对应序号的 1 个 receiver。静态方法、构造器调用和静态字段没有可改写 receiver，会在转换阶段失败。

`@WrapOperation` 用于把匹配方法调用、`invokedynamic` 调用、构造器调用、字段读取、字段写入、数组元素读写、数组长度读取、类型转换、类型判断、局部变量读取或待写入值、条件跳转、switch selector、常量读取或即将抛出的异常替换为 handler，并通过 `Operation`
...
调用模式可在 `At.args` 中声明唯一的 `ldc=<string>` 或 `string=<string>` 直接字符串实参过滤，过滤后再计算 `ordinal` 与命中数；
...
`Operation<Int>`；非字段来源的裸 `ARRAYLENGTH` 使用 `ARRAY_LENGTH`，不使用 `At.target` 或 `At.args`，handler 接收 `Any` / `Object` 数组引用与 `Operation<Int>`，`operation.call(array)` 返回原长度；局部变量读取模式通过 `LOAD` 指定，不使用 `At.target`，可用 `args = ["index=N"]`、`["var=N"]` 或 `["name=localName"]` 按 JVM 局部变量槽位或 LocalVariableTable 变量名过滤，handler 接收 `xLOAD` 读取出的栈顶表达式值与 `Operation<T>`，`operation.call(original)` 会返回传入的原读取值，handler 返回值只替换这一次读取结果，不写回槽位；局部变量写入模式通过 `STORE` 指定，不使用 `At.target`，可用同样的槽位或名称过滤，handler 接收 `xSTORE` 消费前的待写入栈顶值与 `Operation<T>`，`operation.call(original)` 会返回传入的待写入值，handler 返回值交给原 `xSTORE` 继续写入槽位；名称过滤依赖目标 class 保留调试变量表，缺失时不会命中，应改用槽位或 `ordinal`；类型转换模式通过 `CAST` 与类型目标指定，handler 接收待转换对象与 `Operation<T>`，
...
`operation.call(original)` 会返回原始分支结果；handler 返回 `Boolean` 决定是否跳到原标签，`GOTO` 与 `JSR` 不支持包裹。switch 模式通过 `SWITCH` 指定，handler 接收原始 `Int` selector 与 `Operation<Int>`，
`operation.call(selector)` 会返回原始 selector；handler 返回 `Int` 决定原 `tableswitch` 或 `lookupswitch` 的分派，且不支持 `At.target`。常量模式通过 `CONSTANT` 与常量文本目标指定，handler 接收原始常量值与 `Operation<T>`，
...
`FIELD_ASSIGN`、`ARRAY_LENGTH`、`NEW`、`CAST`、`INSTANCEOF`、`LOAD`、`STORE`、`JUMP`、`SWITCH`、`CONSTANT`、`THROW` 操作包裹可用 `Slice` 限制匹配范围；`from` 边界之后、`to` 边界之前的操作才会参与匹配，边界本身不会被包裹，
...
handler 不接收 receiver，`operation.call(...)` 同样只传目标方法参数。`method` 为空时会先按 handler 名称与精确描述符推断目标方法，必要时再按参数/返回兼容规则匹配唯一同名目标；多个兼容重载需要显式指定 `method`。该注解会把原方法体迁移到 `private synthetic` 方法，
再用原方法名与原描述符生成 wrapper；wrapper 会保留原目标方法的 `static`、`final`、`synchronized`、`strictfp` 与 `varargs` 等 JVM 方法契约，因此不支持构造器、类初始化器、abstract 方法或 native 方法。
关键整方法包裹可设置 `require` / `allow` / `expect`，实际命中数按目标方法计数。

`@WrapWithCondition` 用于保留原普通调用、`invokedynamic` 调用、字段读取、字段写入、数组元素读取、
数组元素写入、数组长度读取、对象构造结果、局部变量读取或写入、类型转换、类型判断、常量加载、条件跳转、switch selector 或抛异常点，
但按条件跳过副作用，或在调用完成后按条件保留调用返回值、字段读取结果、数组读取结果、数组长度结果、构造表达式结果、局部状态读取/写入、类型转换结果、类型判断结果、分支跳转、switch 分派或原异常抛出的场景。
handler 返回 `true` 时继续执行原操作、保留表达式结果、沿用原跳转或继续原抛出；返回 `false` 时，调用前、写入、跳转或抛出类控制点会跳过原操作，
调用返回值、字段读取、数组读取、数组长度、构造结果、局部变量读取、类型转换、类型判断、常量与 switch selector 等表达式控制点会替换为对应默认值。调用模式下 handler 先接收原调用
receiver（仅实例调用）和调用参数，字段写入模式下 handler 先接收字段 owner（仅实例字段）和待写入值。
调用返回值模式通过 `INVOKE_ASSIGN` 指定，handler 先接收非 `void` 调用完成后的返回值，返回 `true` 时保留该返回值，返回 `false` 时用返回类型默认值替换本次返回表达式；该模式不会跳过原调用，适合必须保留调用副作用的场景。
...
非字段来源的裸 `ARRAYLENGTH` 使用 `ARRAY_LENGTH`，不使用 `At.target` 或 `At.args`，handler 同样只接收 `Int` 长度值，
...
读取出的值，返回 `false` 时用该值类型默认值替换本次读取结果且不回写槽位；局部变量写入模式通过 `STORE` 与同样的槽位或名称过滤候选，handler 先接收 `xSTORE`
即将消费的待写入值，返回 `false` 时丢弃该值并跳过原局部变量写入，
对象构造模式通过 `NEW` 与可选类型目标指定，handler 先接收构造完成后的引用，
返回 `true` 时保留该对象，返回 `false` 时把本次构造表达式替换为 `null`。
类型转换模式通过 `CAST` 与可选类型目标指定，handler 先接收 `CHECKCAST` 完成后的引用，
返回 `true` 时保留该引用，返回 `false` 时把本次转换结果替换为 `null`。
类型判断模式通过 `INSTANCEOF` 与可选类型目标指定，handler 先接收原始判断结果 `Boolean`，
返回 `true` 时保留原判断结果，返回 `false` 时把本次判断替换为 `false`。
条件跳转模式通过 `JUMP` 与可选跳转操作码目标指定，handler 先接收原始分支结果 `Boolean`，
返回 `true` 时保留原分支结果，返回 `false` 时跳过原跳转，`GOTO` 与 `JSR` 不支持条件包裹。
switch 模式通过 `SWITCH` 指定，handler 先接收原始 `Int` selector，返回 `true` 时保留原 selector，返回 `false` 时把本次 selector 替换为 `0`；`SWITCH` 不支持 `At.target`。
`THROW` 模式匹配 `ATHROW` 前即将抛出的 `Throwable`，handler 先接收该异常对象，返回 `true` 时恢复原异常并继续原 `ATHROW`，
返回 `false` 时跳过该 `ATHROW` 并继续后续字节码；指定异常类型目标时，只匹配 `ATHROW` 前一条真实指令为同类型 `<init>` 的直接构造异常，不追踪局部变量或方法返回值来源。
`CONSTANT` 模式会保留原常量加载指令并在其后插入 `LOAD` 风格条件包裹；handler 首参接收原常量值，返回 `true` 时保留原值，返回 `false` 时压入同类型默认值替换本次常量表达式。显式 `At.target` 时按常量文本过滤，省略 `At.target` 时按 handler 签名筛选兼容常量。
后续都可接收目标方法参数前缀。省略 `INVOKE` 调用目标时，框架会按 handler 参数和 boolean 返回类型筛选兼容的 `void`
或非 `void` 普通调用、`invokedynamic` 调用；返回 `false` 跳过非 `void` 调用时会留下返回类型对应的默认值。构造器和 handler 不兼容的调用不计入 `ordinal` 或命中数。
省略 `INVOKE_ASSIGN` 调用目标时，框架会按 handler 首参与 `Boolean` 返回类型筛选兼容的非 `void` 调用返回值，`void` 或不兼容候选不计入 `ordinal` 或命中数。
省略 `FIELD` 字段目标时，框架会按 handler 首参与 `Boolean` 返回类型筛选兼容字段读取，
不兼容字段读取不计入 `ordinal` 或命中数。
省略 `FIELD_ASSIGN` 字段目标时，框架会按 handler 字段 owner 参数、待写入值和 boolean 返回类型筛选兼容字段写入，
不兼容字段写入不计入 `ordinal` 或命中数。
省略 `NEW` 类型目标时，框架会按 handler 首参与 `Boolean` 返回类型匹配切片内全部兼容构造点，
不兼容构造点不计入 `ordinal` 或命中数。
省略 `CAST` 类型目标时，框架会按 handler 首参与 `Boolean` 返回类型匹配切片内全部兼容类型转换，
不兼容类型转换不计入 `ordinal` 或命中数。
...
局部变量写入、对象构造、类型转换、类型判断、常量加载、条件跳转、switch 或抛异常点才会参与匹配，边界本身不会被包裹，`ordinal` 会在切片内重新计数。
...
`@ModifyExpressionValue` 用于保留原调用、字段读取、字段写入、数组读取、数组写入、字段数组长度、裸 `ARRAYLENGTH`、对象构造、类型转换、类型判断、局部变量读取、局部变量写入、条件跳转、switch、常量加载或抛异常指令但
改写表达式结果的场景，handler 第一个参数接收匹配普通调用或 `invokedynamic` 调用返回值、字段读取值、字段待写入值、数组元素读取值、数组元素待写入值、数组长度 `Int` 值、已初始化对象、转换后的对象、`INSTANCEOF` 判断结果、局部变量读取表达式值、局部变量待写入表达式值、条件跳转分支结果、`tableswitch` / `lookupswitch` 的 `Int` selector、常量表达式值或即将抛出的 `Throwable` 并
...
`INVOKE` / `INVOKE_ASSIGN` 可在 `At.args` 中声明唯一的 `ldc=<string>` 或 `string=<string>` 直接字符串实参过滤，过滤后再计算 `ordinal` 与命中数；
字段读取省略目标时，会按 handler 首参与返回类型筛选兼容的 `GETFIELD` / `GETSTATIC` 字段读取值；字段写入模式通过 `FIELD_ASSIGN` 指定，handler 接收 `PUTFIELD` / `PUTSTATIC` 消费前的待写入值，返回的新值交给原字段写入继续执行；省略字段目标时，会按 handler 首参与返回类型筛选兼容的字段写入候选；
`NEW` 模式会在对应 `<init>` 完成后改写对象表达式，省略类型目标时按 handler 首参与返回类型筛选兼容 `NEW`；
`CAST` 模式会在 `CHECKCAST` 后改写类型转换结果，省略类型目标时按 handler 首参与返回类型筛选兼容 `CHECKCAST`；`CONSTANT` 模式会在常量加载后改写常量表达式值，省略常量目标时按 handler 首参与返回类型筛选兼容常量；`LOAD` 模式不使用 `At.target`，可用 `at.args = ["index=N"]`、`["var=N"]` 或 `["name=localName"]` 按 JVM 局部变量槽位或 LocalVariableTable 变量名过滤，handler 接收 `xLOAD` 读取出的栈顶表达式值，返回值只替换这一次读取结果，不写回原槽位；`STORE` 模式同样不使用 `At.target`，可用槽位或名称过滤，handler 接收 `xSTORE` 消费前的待写入栈顶值，返回值交给原 `xSTORE` 继续写入槽位；名称过滤依赖目标 class 保留调试变量表，缺失时不会命中，应改用槽位或 `ordinal`；不兼容的调用返回、字段读取、字段写入、数组写入、`NEW`、`CHECKCAST`、`LOAD`、`STORE` 或常量候选不计入 `ordinal` 或命中数。数组读取模式通过 `args = ["array=get"]`
指定，数组写入值模式通过 `FIELD_ASSIGN + args = ["array=set"]` 指定，handler 接收 `xASTORE` 消费前的待写入元素值，不接收数组引用或索引，返回的新值交给原数组写入继续执行；字段数组长度模式通过 `FIELD + args = ["array=length"]` 指定并接收 `Int` 长度；非字段来源的数组长度使用 `At(value = InjectionPoint.ARRAY_LENGTH)`，直接匹配任意裸 `ARRAYLENGTH` 指令，不使用 `At.target`，handler 同样接收 `Int` 长度。`INSTANCEOF` 模式接收 `Boolean` 判断结果，`JUMP` 模式接收条件跳转原始分支结果并返回新的 `Boolean` 分支结果，`GOTO` 与 `JSR` 不支持表达式改写，`SWITCH` 模式不支持 `At.target`，接收并返回 `Int` selector，可同时改写 `tableswitch` 与 `lookupswitch` 分派，`THROW` 模式接收即将抛出的 `Throwable`，且后续参数仍可接收目标方法参数前缀；`THROW` 指定类型目标时，只匹配 `ATHROW` 前一条真实指令为同类型 `<init>` 的直接构造异常，不追踪局部变量或方法返回值来源。
...
`INVOKE` / `INVOKE_ASSIGN` 调用返回、字段读取、字段写入值、数组元素读取、数组元素写入值、字段数组长度、裸 `ARRAYLENGTH`、`NEW`、`CAST`、`INSTANCEOF`、`LOAD`、`STORE`、`JUMP`、`SWITCH`、`CONSTANT` 与 `THROW` 表达式值修改可用
`Slice` 限制匹配范围；`from` 边界之后、`to` 边界之前的调用、字段读取、字段写入、数组读取、数组写入、数组长度、对象构造、类型转换、类型判断、局部变量读取、局部变量写入、条件跳转、switch 或抛异常候选点才会参与匹配，
边界本身不会被改写，`THROW` 只会改写边界内的 `ATHROW` 候选，`ordinal` 会在切片内重新计数。
关键表达式值补丁可设置 `require` / `allow` / `expect`，命中数按实际改写的表达式值数量计数。

`@ModifyVariable` 支持 `HEAD` 入口参数改写、`LOAD` 局部变量读取前改写和 `STORE` 局部变量写入后改写。`HEAD` 适合在方法体执行前重写参数值；`LOAD` 会在匹配的 `xLOAD` 指令前读取当前局部变量，调用 handler，并写回同一槽位；`STORE` 会在匹配的 `xSTORE` 指令后读取刚写入的局部变量，调用 handler，并写回同一槽位。`LOAD` / `STORE` 模式可用 `Slice` 限制读取点或写入点匹配范围；`from` 边界之后、`to` 边界之前的读取或写入才会参与匹配，边界本身不会被修改，`ordinal` 会在切片内重新计数。`@ModifyVariable` handler 第一个参数接收原变量值并返回同类型的新值；显式指定 `index` 时，对象或数组变量可声明为原值类型的父类、接口、`Any` 或 `Object` 接收，框架会尽量保留真实槽位类型再写回，避免局部变量栈图退化为 `Object`。返回值对引用类型可为原变量类型的可赋值子类型，也可用 `Any` 或 `Object` 作为泛型引用返回类型，框架会在 handler 调用后转换回原变量类型，基础类型仍需精确匹配。后续参数可继续接收目标方法参数前缀；对象或数组目标方法参数同样可声明为原值类型的父类、接口、`Any` 或 `Object`。`@ModifyVariable.index` 使用 JVM 局部变量槽位索引，实例方法槽位 0 是 `this`，第一个参数从槽位 1 开始；静态方法第一个参数从槽位 0 开始。`argsOnly = true` 会把候选限制为目标方法参数槽位，适合只想改参数但方法体内部有同类型临时变量的场景；显式 `index` 指向非参数槽位时不会命中，并会按 `require` / 默认命中数契约失败。`@ModifyVariable.name` 可按 LocalVariableTable 中的变量名继续过滤候选，能与 `index`、`argsOnly`、类型和 `ordinal` 组合使用；目标 class 缺少调试变量表时，名称筛选不会命中，应改用槽位、`argsOnly` 或序号。未指定 `index` 时，会按 handler 第一个参数类型筛选入口参数、读取点或写入点；`HEAD` 模式同类型入口参数唯一时可自动推断，存在多个同类型入口参数或在 `LOAD` / `STORE` 模式选择同类型读写点时，用 `ordinal` 选择第 N 个同类型匹配项，因此应使用实际变量类型参与自动匹配。省略 `method` 时，handler 名称必须与目标方法名一致，框架会按变量类型、返回类型、`index` / `name` / `argsOnly` / `ordinal`、`slice` 限定后的实际读写候选和追加目标参数匹配唯一同名目标方法；`LOAD` / `STORE` 会用真实 `xLOAD` / `xSTORE` 候选参与重载推断，`name` 可用于区分保留 LocalVariableTable 的重载方法，多个兼容重载仍需要显式指定完整方法签名。关键变量补丁可设置 `require` / `allow` / `expect`，`HEAD` 按入口改写计 1 次，`LOAD` / `STORE` 按实际改写的读写点数量计数。`HEAD` 当前不使用 `slice`。
...
无参 handler，适合记录日志、打点或触发旁路逻辑。普通注入不会自动把局部变量值传给 handler，也不会写回变量；
只读观察同作用域局部变量时可显式使用 `@Local`；
可以用 `Slice` 把候选读写指令限制在一段 `INVOKE`、`FIELD`、`FIELD_ASSIGN` 或 `CONSTANT` 边界内，也可以用
...
`@ModifyReturnValue` 默认会修改目标方法的全部非 void 返回点；`Slice` 可把候选返回点限制在 `from` 边界之后、`to` 边界之前，边界本身不会参与匹配；`ordinal` 可限定只修改第 N 个返回点，并会在切片内重新计数。`at` 参数当前仅预留，不能把修改位置改到其他表达式。
handler 返回类型对 primitive 必须与目标方法返回类型一致；对象或数组返回值可为目标类型的子类型，也可用 `Any` 或 `Object` 作为泛型引用返回类型，框架会在 handler 调用后转换回目标返回类型。handler 可以不声明参数并直接返回新值；对象或数组返回值可用原值类型的父类、接口、`Any` 或 `Object` 接收原返回值，后续也可接收目标方法参数前缀。省略 `method` 时会按 handler 名称、签名兼容性、`slice` 限定后的真实返回点和 `ordinal` 推断唯一同名目标方法；多个兼容重载或都拥有对应返回点时需要显式写出目标签名。
...
object RedirectMixin {
    ...
    @Redirect(
        method = "readCachedRoute(Ljava/lang/String;)Ljava/lang/String;",
        at = At(value = InjectionPoint.LOAD, args = ["index=2"]),
        ordinal = 0,
    )
    fun redirectRouteLoad(original: String, profile: String): String =
        "$profile:$original"

    @Redirect(
        method = "storeRoute(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;",
        at = At(value = InjectionPoint.STORE, args = ["index=3"]),
        ordinal = 0,
    )
    fun redirectRouteStore(original: String, profile: String, route: String): String =
        "$profile:$route:$original"
...
非 `void` 目标方法可直接在可取消 HEAD、TAIL 或普通 `INVOKE` / `INVOKE_ASSIGN` 的 `BEFORE` / `AFTER`
注入中调用 `callback.setReturnValue(value)`，该调用会同时取消后续目标方法逻辑。

`@WrapWithCondition` 可用于按条件跳过普通调用、`invokedynamic` 调用，也可在调用完成后按条件保留返回值，并支持字段读取、字段写入、数组元素读取、数组元素写入、数组长度读取、局部变量读取或写入、对象构造结果、类型转换、类型判断、常量加载、条件跳转、switch selector 或即将抛出的异常。调用返回值模式使用
`At(value = InjectionPoint.INVOKE_ASSIGN, target = "...")` 精确匹配非 `void` 调用；handler 首参接收调用返回值，返回 `false` 时用返回类型默认值替换本次返回表达式，且不会跳过原调用。
字段读取模式使用
`At(value = InjectionPoint.FIELD, target = "...")` 精确匹配字段，也可省略字段目标按 handler 首参与 `Boolean` 返回类型筛选兼容读取；handler 首参接收 `GETFIELD` / `GETSTATIC`
...
不接收数组引用，返回 `false` 时用 `0` 替换本次数组长度结果；非字段来源的裸 `ARRAYLENGTH` 使用
`At(value = InjectionPoint.ARRAY_LENGTH)`，不使用 `At.target` 或 `At.args`，handler 同样接收 `Int` 长度值。数组元素写入使用
`At(value = InjectionPoint.FIELD_ASSIGN, target = "...", args = ["array=set"])`，handler 参数为数组引用、
`Int` 索引与待写入元素值，返回 `false` 时跳过原 `xASTORE`。
局部变量读取使用 `At(value = InjectionPoint.LOAD, args = ["index=N"])`、`["var=N"]` 或 `["name=localName"]`
过滤候选，handler 先接收 `xLOAD` 读取值，返回 `false` 时用默认值替换本次读取结果且不回写槽位；局部变量写入使用
`At(value = InjectionPoint.STORE, args = ["index=N"])`、`["var=N"]` 或 `["name=localName"]` 过滤候选，handler 先接收 `xSTORE` 待写入值，返回 `false` 时跳过原 `xSTORE`。
对象构造模式使用 `At(value = InjectionPoint.NEW, target = "...")` 精确匹配构造类型，也可省略类型目标匹配切片内全部兼容构造点；handler 先接收构造完成后的引用，返回 `true` 时保留该对象，返回 `false` 时把本次构造表达式替换为 `null`。
类型转换模式使用 `At(value = InjectionPoint.CAST, target = "...")` 精确匹配类型，也可省略类型目标匹配切片内全部兼容类型转换；handler 先接收 `CHECKCAST` 完成后的引用，返回 `true` 时保留转换结果，返回 `false` 时把本次转换结果替换为 `null`。
类型判断模式使用 `At(value = InjectionPoint.INSTANCEOF, target = "...")` 精确匹配类型，也可省略类型目标匹配切片内全部兼容类型判断；handler 先接收原始判断结果 `Boolean`，返回 `true` 时保留原判断结果，返回 `false` 时把本次判断替换为 `false`。
条件跳转模式使用 `At(value = InjectionPoint.JUMP, target = "...")` 精确匹配条件跳转，也可省略跳转目标匹配切片内全部条件跳转；handler 先接收原始分支结果 `Boolean`，返回 `true` 时保留原分支结果，返回 `false` 时跳过原跳转，`GOTO` 与 `JSR` 不支持条件包裹。
...
object CalculatorMixin {
    ...
    fun modifyResult(callback: CallbackInfoReturnable<Int>) {
        callback.value = callback.getReturnValueI() * 2
    }
}
```
...
在 `RETURN` 注入中，`setReturnValue` 只修改当前返回点；在 `cancellable = true` 的 `HEAD`、`TAIL`
或普通 `INVOKE` / `INVOKE_ASSIGN` 的 `BEFORE` / `AFTER` 注入中，`setReturnValue` 会同时取消后续目标方法逻辑并作为提前返回值。
引用类型返回值可以被设置为 `null`，并会作为明确的新返回值写回。
...
fun normalizeArguments(args: Args) {
    args.setAll(args.get<String>(0), args.get<String>(1).trim())
}
...
fun rewriteLabel(original: String): String = "relay-$original"
...
fun keepBranch(original: Boolean): Boolean = original
...
省略调用目标时，框架会按 handler 签名筛选兼容的普通调用、构造器调用或动态调用，非兼容调用不会参与 `ordinal` 和命中数统计。
若只想替换带固定直接字符串实参的调用点，可在 `At.args` 中声明唯一的 `ldc=<string>` 或 `string=<string>`，
该过滤只检查调用参数来源，不匹配 receiver、局部变量、拼接字符串、方法返回值或 bootstrap 常量。
只替换带固定直接字符串实参调用点的 receiver 时，`@ModifyReceiver(INVOKE)` 可使用同样的 `At.args` 过滤；
过滤后再计算 `ordinal` 与命中数，字段 receiver 模式不使用该字符串过滤，携带 `At.args` 会在转换阶段失败。
...
显式目标方法只存在于旧版本时，当前版本缺失会按 0 命中参与组级累计；但整个组必须满足 `min` / `max`。
未分组处理器仍保持默认必须命中的旧契约：
