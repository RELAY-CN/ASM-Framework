...
返回同类型新值，后续参数可接收目标方法参数前缀；`THROW` 模式可返回 `Throwable` 或具体异常子类；对象或数组表达式的首参可声明为 `Any` / `Object`，
除 `THROW` 外返回类型仍需保持表达式类型；它不会接收原调用参数、`GETFIELD` receiver、数组引用或
