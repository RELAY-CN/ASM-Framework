...
- **@WrapWithCondition** - 按条件跳过普通调用、`invokedynamic` 调用、字段读取、字段写入、数组元素读取、数组元素写入、数组长度读取、局部变量读取或写入、类型判断、常量加载、条件跳转或即将抛出的异常
...
普通 `@AsmInject(SWITCH)` 只在 `tableswitch` 或 `lookupswitch` 指令前后插入 handler，不接收也不改写 selector，且不支持 `At.target`。需要直接替换 switch selector 时可使用 `@Redirect(SWITCH)`；需要保留原指令并改写 selector 时可使用 `@ModifyExpressionValue(SWITCH)`；需要保留可调用原 selector 的操作句柄时可使用 `@WrapOperation(SWITCH)`。
...
数组元素写入、数组长度读取、局部变量读取或写入、类型判断、常量加载、条件跳转或抛异常点，
但按条件跳过副作用、字段读取结果、数组读取结果、数组长度结果、局部状态读取/写入、类型判断结果、分支跳转或原异常抛出的场景。
...
`LOAD`、`STORE`、`INSTANCEOF`、`CONSTANT`、`JUMP` 与 `THROW` 条件包裹可用 `Slice` 限制匹配范围；`from` 边界之后、
`to` 边界之前的调用、字段读取、字段写入、数组元素读取、数组元素写入、数组长度读取、局部变量读取、
局部变量写入、类型判断、常量加载、条件跳转或抛异常点才会参与匹配，边界调用本身不会被包裹，`ordinal` 会在切片内重新计数。
...
`@WrapWithCondition` 可用于按条件跳过普通调用、`invokedynamic` 调用、字段读取、字段写入、数组元素读取、数组元素写入、数组长度读取、局部变量读取或写入、类型判断、常量加载、条件跳转或即将抛出的异常。字段读取模式使用
