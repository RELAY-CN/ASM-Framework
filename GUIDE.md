...
只有声明 `cancellable = true` 的可取消回调才能调用 `CallbackInfo.cancel()`；当前 `HEAD` 与 `TAIL` 注入会在取消后提前返回。
