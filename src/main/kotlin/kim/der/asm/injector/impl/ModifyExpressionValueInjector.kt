 ...
 * [InjectionPoint.INSTANCEOF]、[InjectionPoint.LOAD]、[InjectionPoint.STORE]、[InjectionPoint.JUMP]、[InjectionPoint.SWITCH]、[InjectionPoint.CONSTANT] 与 [InjectionPoint.THROW] 表达式可使用
...
class ModifyExpressionValueInjector(
    ...
    /**
     * 判断当前表达式匹配序号是否满足用户声明的 ordinal 过滤。
     *
     ...
     * 任一边界声明但无法命中时返回空范围，避免目标字节码漂移后误改写切片外表达式。
     *
     * @param insns 目标方法指令数组
     ...
     * 提取调用指令的可读名称。
     *
     * 该名称只用于错误提示，非调用指令返回占位文本。
     *
     * @param insn 待描述的指令
     * @return 普通调用或动态调用名称；非调用指令返回 `<unknown>`
     */
