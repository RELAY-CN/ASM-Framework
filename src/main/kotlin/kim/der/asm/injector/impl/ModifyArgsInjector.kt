...
class ModifyArgsInjector(
    ...
    /**
     * 判断当前匹配调用点序号是否满足用户声明的 ordinal 过滤。
     *
     * @param currentOrdinal 已通过目标与切片过滤的调用点序号
     ...
     * 任一边界声明但无法命中时返回空范围，避免目标字节码漂移后误修改切片外调用点。
     *
     ...
     * 从指定位置开始查找切片边界调用指令。
     *
     * 当前 `@ModifyArgs` 的切片边界只支持 [InjectionPoint.INVOKE]，
     ...
     * 在显式声明目标时描述匹配到的调用点。
     *
     * 普通实例调用会记录需要保存和恢复 receiver；静态调用与 `invokedynamic` 不需要 receiver。
     *
     * @param insn 待检查的指令
     * @param targetOwner 目标 owner；为 `null` 时不限制 owner
     * @param targetName 目标方法名或动态调用名
     * @param targetDesc 目标调用描述符
     * @return 匹配调用点的信息；不匹配时返回 `null`
     */
     ...
     * 在未显式声明目标时描述任意可处理调用点。
     *
     * 该路径用于 `At.target` 为空的自动推断模式，会接受普通方法调用、构造器调用与 `invokedynamic`。
     *
     * @param insn 待检查的指令
     * @return 可处理调用点的信息；非调用指令返回 `null`
     */
     ...
     * 调用点参数组改写所需的最小调用点信息。
     *
     * @property desc 调用点描述符
     * @property hasReceiver 调用点栈上是否包含需要保存和恢复的 receiver
     */
