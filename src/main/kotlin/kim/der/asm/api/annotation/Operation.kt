 ...
 * 原始调用操作句柄。
 *
 * [WrapOperation] handler 会接收该对象，用于按需执行被包裹的原始方法调用、构造器调用、字段读取、字段写入、
 * 数组元素读写或数组长度读取。
 ...
 * 数组实例、索引与新元素值，数组长度读取只需传入数组实例。构造器调用只传入构造器参数，不传入未初始化 receiver。
 *
...
class Operation<T> private constructor(
     ...
     * 数组实例、`Int` 索引与新元素值，数组长度读取的 [args] 必须只包含数组实例。返回值类型由调用方的
     * handler 签名决定。
     *
