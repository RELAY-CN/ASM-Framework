 ...
 * [InjectionPoint.INVOKE] 注入，以及 [Redirect] 的普通方法调用重定向支持 [from] / [to]
 * 为 [InjectionPoint.INVOKE] 的边界；起始边界之后、结束边界之前的调用点才会参与匹配，
 * 边界指令本身不会作为候选注入点。[AsmInject.ordinal] / [Redirect.ordinal] 会在切片内重新计数。
 * 指定的边界未命中时，切片按空范围处理。
 *
 * @param from 起始定位条件
 * @param to 结束定位条件
