 ...
 * 包裹原始操作注解。
 *
 * 用于把目标方法内匹配的方法调用、构造器调用、字段读取、字段写入、数组元素读写、数组长度读取或类型转换替换为 handler 调用（语义参考
 * Mixin Extras 的 `@WrapOperation`）。handler 会接收原操作的 receiver（实例调用、实例字段读取与
 * 实例字段写入）、原调用参数、构造器参数、字段写入值、数组访问参数或类型转换输入值与 [Operation]，
 * 可选择调用、跳过或多次调用原始操作。
 *
 ...
 * - 下一参数必须是 [Operation]，用于执行原始调用、构造器调用、字段读取、字段写入、数组元素读写、数组长度读取或类型转换
 ...
 *   或 [InjectionPoint.CAST]，并通过 [At.target] 指定要匹配的方法调用、字段读取、字段写入、产生数组引用的字段或类型转换目标类型
 *
 * @param method 目标方法签名
 ...
 * [InjectionPoint.FIELD_ASSIGN]、[InjectionPoint.CAST] 操作包裹支持用 [Slice.from] / [Slice.to] 的 [InjectionPoint.INVOKE] 边界缩小查找范围
 ...
 * - 显式指定 [index] 时，对象/数组变量或目标方法参数对应的 handler 参数可声明为 `Any` / `Object`，但返回类型仍需保持变量类型
...
/**
 * 重定向方法调用、构造器调用、字段访问、类型转换或类型判断注解。
 *
 * 用于将目标方法中的某个方法调用、构造器调用、字段读取、字段写入、简单数组元素访问、数组长度读取、`CHECKCAST` 类型转换或 `INSTANCEOF`
 ...
 * 重定向处理器要求：
 *
 * - 方法调用、构造器调用、字段读取、字段写入、数组元素访问、数组长度、类型转换与类型判断重定向支持静态方法、`@JvmStatic` 方法，或 Kotlin `object` 中的实例方法
 ...
 *
 * @param method 目标方法签名
 * @param target 目标调用、构造器、字段或类型签名组件；会与 [At.target] 组合构建最终的匹配签名
 * @param at 调用点信息；[At.value] 决定重定向方法调用、构造器调用、字段读取、字段写入、数组元素访问、数组长度读取、类型转换还是类型判断，[At.target] 用于指定匹配签名
 * @param ordinal 匹配点序号；`-1` 表示重定向全部匹配点，当前在方法调用、构造器调用、字段读取、字段写入、数组元素访问、数组长度读取、类型转换与类型判断中生效
 * @param slice 切片范围；当前方法调用、构造器调用、字段读取、字段写入、数组元素访问、数组长度、类型转换与类型判断重定向
 * 支持用 [Slice.from] / [Slice.to] 的 [InjectionPoint.INVOKE] 边界缩小查找范围
