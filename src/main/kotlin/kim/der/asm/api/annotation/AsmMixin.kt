 ...
 * @param priority 应用优先级；数值越高越先应用，同优先级保持注册顺序
 *
...
annotation class AsmMixin(
     ...
     * Mixin 应用优先级。
     *
     * 默认值与常见 Mixin 风格保持一致。同一匹配来源内数值越高越先应用；
     * 数值相同时保持注册顺序，路径匹配来源仍整体先于精确目标来源。
     */
 ...
 * 单个处理器允许 0 命中，但整个分组必须至少命中 [min] 次。
 *
 * ## 使用边界
 *
 ...
 * - [expect] 只输出警告，不阻断转换；[min] 与 [max] 会在转换阶段失败。
 *
 * @param name 分组名称；同名处理器共享同一个命中数契约
 * @param min 分组最小命中数；实际总命中数不足时转换失败
 * @param max 分组最大命中数；实际总命中数超过时转换失败
 * @param expect 分组期望命中数；非负且不一致时只输出警告
 *
 * @author Dr (dr@der.kim)
 * @date 2025-11-24
 */
...
annotation class Group(
     ...
     * 分组名称。
     *
     * 名称用于在同一个 Mixin 类内合并多个处理器的实际命中数，不能为空白。
     */
    val name: String,
     ...
     * 分组最小命中数。
     *
     * 默认至少需要同组任一候选命中一次，适合多版本二选一补丁；该值必须非负。
     */
    val min: Int = 1,
     ...
     * 分组最大命中数。
     *
     * 默认不限制最大命中数；可设为 `1` 表示多个候选中只允许一个命中，且不得小于 [min]。
     */
     ...
     * 分组期望命中数。
     *
     * `-1` 表示不检查；非负且实际命中数不一致时只输出警告，不阻断转换。
     */
    val expect: Int = -1,
)
 ...
 * 显式 [At.target] 可写 `owner.name(desc)`、`owner/name(desc)` 或 `name(desc)`；省略 owner 时只按方法名与描述符匹配，
 * 缺少方法描述符的显式目标会在转换阶段失败。
 ...
 * `invokedynamic` 调用，为空则按 handler 签名推断兼容调用点；显式目标可写 `owner.name(desc)`、`owner/name(desc)` 或
 * `name(desc)`，但必须包含方法描述符，缺失时会在转换阶段失败；`invokedynamic` 目标按 bootstrap owner、动态调用名或 bootstrap 名，
 ...
 * - handler 必须返回 `void`
 * - 后续参数可按顺序接收目标方法参数前缀
 * - [At.value] 必须为 [InjectionPoint.INVOKE]；[At.target] 可指定要匹配的方法调用、构造器调用或 `invokedynamic` 调用，也可省略并按兼容调用点推断
 ...
 * [At.target] 为空时按兼容调用点推断；显式目标可写 `owner.name(desc)`、`owner/name(desc)` 或 `name(desc)`，
 * 但必须包含方法描述符，缺失时会在转换阶段失败；可用 [At.args] 中唯一的 `ldc=<string>` 或 `string=<string>` 过滤直接字符串实参调用点
 ...
 * 可省略 [method]，框架会按 handler 方法名、receiver 操作点、handler 签名和后续目标方法参数前缀，
 * 在目标类中推断唯一同名方法；[At.target] 为空时会使用实际可兼容的 receiver 候选参与推断，
 * 若存在多个兼容重载，需要显式声明 [method]。
 *
 * ASM 方法要求：
 *
 ...
 *
 * @param method 目标方法签名；可省略并按 handler 名称、receiver 操作点、handler 签名和实际兼容候选推断唯一兼容目标
 ...
 * [InjectionPoint.ARRAY_LENGTH] 裸数组长度读取、[InjectionPoint.THROW] 抛异常操作；[InjectionPoint.FIELD] 可通过 `args = ["array=get"]`
 ...
 * 切片内全部 `INSTANCEOF` 判断。
 * 局部变量读取通过 [InjectionPoint.LOAD] 指定，不使用 [At.target]；可通过 [At.args] 中的 `index=N`、`var=N`
 * 或 `name=localName` 按 JVM 局部变量槽位或 LocalVariableTable 变量名过滤，handler 返回值只替换这一次读取结果，不写回原槽位。
 * 局部变量写入通过 [InjectionPoint.STORE] 指定，不使用 [At.target]；可通过同样的槽位或名称过滤指定候选，
 * handler 返回值交给原 `xSTORE` 继续写入槽位。
 * 条件跳转通过 [InjectionPoint.JUMP] 与跳转操作码名或数字指定；省略 [At.target] 时会匹配切片内全部条件跳转，`GOTO` 与 `JSR` 不支持包裹。
 * switch selector 通过 [InjectionPoint.SWITCH] 指定；不支持 [At.target]，会匹配 `tableswitch` 与 `lookupswitch` 消费前的 `Int` selector。
 ...
 * - 局部变量读取 handler 参数先接收 `xLOAD` 读取出的栈顶表达式值；`Operation.call(value)` 会返回传入的原读取值
 * - 局部变量写入 handler 参数先接收 `xSTORE` 消费前的待写入栈顶值；`Operation.call(value)` 会返回传入的待写入值
 * - 条件跳转 handler 参数先接收原始分支结果；`Operation.call(original)` 会返回原始分支结果
 * - switch handler 参数先接收原始 `Int` selector；`Operation.call(selector)` 会返回原始 selector
 * - 常量 handler 参数先接收原始常量值；`Operation.call()` 会返回原始常量值
 * - 抛异常 handler 参数先接收即将抛出的 [Throwable]；`Operation.call(throwable)` 会返回原异常对象
 * - 下一参数必须是 [Operation]，用于执行原始调用、`invokedynamic` 调用、构造器调用、字段读取、字段写入、数组元素读写、数组长度读取、类型转换、类型判断、局部变量读取、局部变量待写入值透传、条件跳转、switch selector、常量读取或原抛异常操作
 ...
 *   [InjectionPoint.NEW]、[InjectionPoint.CAST]、[InjectionPoint.INSTANCEOF]、[InjectionPoint.LOAD]、[InjectionPoint.STORE]、[InjectionPoint.JUMP]、[InjectionPoint.SWITCH]、[InjectionPoint.CONSTANT]、[InjectionPoint.ARRAY_LENGTH] 或 [InjectionPoint.THROW]，并通过 [At.target]
 *   指定要匹配的方法调用、`invokedynamic` 调用、字段读取、字段写入、产生数组引用的字段、构造类型、类型目标、跳转操作码、常量或异常类型；[InjectionPoint.SWITCH] 不使用 [At.target]，[InjectionPoint.ARRAY_LENGTH] 不使用 [At.target] 或 [At.args]
 ...
 *   [InjectionPoint.STORE] 不使用 [At.target]，可通过同样的槽位或名称过滤局部变量；
 *   [InjectionPoint.JUMP] 可省略 [At.target]，匹配切片内全部条件跳转；
 *   [InjectionPoint.SWITCH] 不支持 [At.target]，匹配切片内全部 `tableswitch` / `lookupswitch`；
 ...
 * [InjectionPoint.NEW]、[InjectionPoint.CAST]、[InjectionPoint.INSTANCEOF]、[InjectionPoint.LOAD]、[InjectionPoint.STORE]、[InjectionPoint.JUMP]、[InjectionPoint.SWITCH]、[InjectionPoint.CONSTANT]、[InjectionPoint.ARRAY_LENGTH] 与 [InjectionPoint.THROW]
 ...
 * - 带 [Group] 的显式 [method] 候选在当前版本缺失时按 0 命中累计；未分组处理器仍会直接失败
 ...
 * Mixin Extras 的 `@WrapWithCondition`）。handler 返回 `true` 时继续执行原操作、保留表达式结果、按原分支结果跳转或继续原抛出；
 * 返回 `false` 时，调用前、写入、跳转或抛出类控制点会跳过原操作，调用返回值、字段读取、数组元素读取、
 * 数组长度读取、裸数组长度读取、局部变量读取、类型判断、常量加载和 switch selector 会留下对应类型的默认值；对象构造结果与类型转换会留下 `null`。
 * 相比 [Redirect]，该注解不替换原逻辑，只决定原操作、表达式结果、原跳转或原抛出是否保留，更适合“按条件跳过副作用调用、写入、分支跳转或异常抛出”的场景。
 *
 ...
 * handler 首参接收已经读取出的数组元素值，不接收数组引用或索引；
 * 返回 `false` 时用元素类型默认值替换本次读取结果。
 * 数组长度读取使用 [InjectionPoint.FIELD]、数组字段 [At.target] 与 [At.args] 中的 `array=length` 指定，
 * handler 首参接收 `Int` 长度值，不接收数组引用；返回 `false` 时用 `0` 替换本次数组长度结果。
 ...
 * 或 `name=localName` 按 JVM 局部变量槽位或 LocalVariableTable 变量名过滤。handler 先接收 `xLOAD` 读取出的值；
 ...
 * [InjectionPoint.INSTANCEOF] 模式匹配类型判断，类型目标支持 internal name 或 binary name。
 * handler 先接收原始 `Boolean` 判断结果；返回 `true` 时保留原判断结果，返回 `false` 时把本次判断替换为 `false`。
 * 省略 [At.target] 时会按 handler 签名匹配切片内全部兼容类型判断，不兼容候选不计入 [ordinal] 或命中数。
 * [InjectionPoint.JUMP] 模式匹配条件跳转，[At.target] 可写条件跳转操作码名或数字；
 * 省略 [At.target] 时匹配切片内全部条件跳转，handler 先接收原始分支结果 `Boolean`，
 * 返回 `true` 时保留原分支结果，返回 `false` 时跳过原跳转；`GOTO` 与 `JSR` 不支持条件包裹。
 * [InjectionPoint.SWITCH] 模式匹配 `tableswitch` / `lookupswitch` 消费前的 `Int` selector，不支持 [At.target]。
 * handler 先接收原始 selector；返回 `true` 时保留原 selector，返回 `false` 时把本次 selector 替换为 `0`。
 * [InjectionPoint.CONSTANT] 模式匹配常量加载指令。转换会保留原常量加载，并在其后插入 LOAD 风格条件包裹；
 * handler 首参接收原常量值，返回 `true` 时保留原值，返回 `false` 时压入同类型默认值。
 * 指定 [At.target] 时按常量文本过滤；省略 [At.target] 时按 handler 首参与 `Boolean` 返回类型筛选兼容常量，
 * 不兼容候选不计入 [ordinal] 或命中数。
 ...
 * [InjectionPoint.INVOKE]、[InjectionPoint.INVOKE_ASSIGN]、[InjectionPoint.FIELD]、[InjectionPoint.FIELD_ASSIGN]、[InjectionPoint.LOAD]、
 ...
 * - 调用返回值的 handler 先接收非 `void` 调用完成后的返回值
 * - 字段读取的 handler 先接收 `GETFIELD` / `GETSTATIC` 读取出的字段值，不接收 receiver
 * - 实例字段写入的 handler 参数先接收字段 owner，再接收待写入值
 * - 静态字段写入的 handler 参数接收待写入值
 * - 数组元素读取的 handler 参数先接收已经读取出的元素值，不接收数组引用或索引
 ...
 * - 条件跳转的 handler 先接收原始分支结果 `Boolean`
 * - switch selector 的 handler 先接收原始 `Int` selector
 * - 抛异常点的 handler 先接收即将抛出的 [Throwable]
 * - 后续参数可按顺序接收目标方法参数前缀
 * - 引用类型参数可声明为精确类型、可赋值父类型或 `Any` / `Object`
 * - [At.target] 可指定要匹配的方法调用、动态调用、字段签名、类型、跳转操作码、常量文本或异常类型；[InjectionPoint.INVOKE]、
 *   [InjectionPoint.INVOKE_ASSIGN]、[InjectionPoint.FIELD]、[InjectionPoint.FIELD_ASSIGN]、[InjectionPoint.NEW]、[InjectionPoint.CAST]、[InjectionPoint.INSTANCEOF]、[InjectionPoint.JUMP]、[InjectionPoint.CONSTANT]
...
annotation class WrapWithCondition(
     ...
     * 条件包裹操作点定位。
     *
     * 支持普通方法调用、调用返回值、字段读取、字段写入、数组元素读取、数组元素写入、数组长度读取、裸数组长度读取、对象构造结果、局部变量读取/写入、类型转换、类型判断、条件跳转、switch selector、常量加载和抛异常点。
     */
    val at: At = At(value = InjectionPoint.INVOKE),
 ...
 * `invokedynamic` 调用完成后的非 `void` 返回值、字段读取值、字段写入值、数组元素读取值、数组元素待写入值、数组长度值、对象构造完成后的实例、
 * `CHECKCAST` 完成后的类型转换结果、`INSTANCEOF` 判断后的 boolean 结果、局部变量读取表达式值、局部变量待写入表达式值、条件跳转的分支结果、`tableswitch` / `lookupswitch` 的 `Int` selector 或常量表达式值，也可在 `ATHROW` 前改写即将抛出的异常对象。
 * 相比 [Redirect]，该注解不替换原调用、字段读取、字段写入、数组读取、数组写入、构造器调用、类型转换、类型判断、局部变量读取、局部变量写入、条件跳转、switch selector、常量加载或抛异常指令，只在表达式产生值后
 * 把原值交给 handler 改写。
 ...
 * - [InjectionPoint.JUMP] 的 [At.target] 为条件跳转操作码名或数字；省略 [At.target] 时匹配切片内全部条件跳转；handler 接收原始分支结果 `Boolean`，返回新的分支结果；`GOTO` 与 `JSR` 不支持表达式改写
 * - [InjectionPoint.SWITCH] 不支持 [At.target]；handler 接收 `tableswitch` 或 `lookupswitch` 消费前的 `Int` selector，返回新的 `Int` selector
 ...
 * @param at 表达式定位；当前支持 [InjectionPoint.INVOKE]、[InjectionPoint.INVOKE_ASSIGN]、[InjectionPoint.FIELD]、[InjectionPoint.FIELD_ASSIGN]、
 * [InjectionPoint.NEW]、[InjectionPoint.CAST]、[InjectionPoint.INSTANCEOF]、[InjectionPoint.LOAD]、[InjectionPoint.STORE]、[InjectionPoint.JUMP]、[InjectionPoint.SWITCH]、[InjectionPoint.CONSTANT]、[InjectionPoint.ARRAY_LENGTH] 与 [InjectionPoint.THROW]
 ...
 * 按 boolean 常量处理；匹配 `ACONST_NULL` 时，handler 的第一个参数可声明为任意引用类型接收原始 `null`。
 *
 * ASM 方法要求：
 *
 ...
 * 局部变量写入重定向通过 [At.value] 指定 [InjectionPoint.STORE]，不使用 [At.target]；可通过同样的槽位或名称过滤指定候选。
 ...
 * 条件跳转重定向通过 [At.value] 指定 [InjectionPoint.JUMP]，并通过 [At.target] 指定条件跳转操作码名或数字；
 * [At.target] 为空时会匹配切片内全部条件跳转，`GOTO` 与 `JSR` 不支持重定向。
 * switch selector 重定向通过 [At.value] 指定 [InjectionPoint.SWITCH]；该模式不使用 [At.target]，会匹配
 * `tableswitch` 与 `lookupswitch` 消费前的 `Int` selector。
 * 常量加载重定向通过 [At.value] 指定 [InjectionPoint.CONSTANT]，并通过 [At.target] 指定常量文本；
 * [At.target] 为空时会按 handler 首参与返回类型筛选兼容常量，不兼容候选不计入 [ordinal] 或命中数。
 * 抛异常重定向通过 [At.value] 指定 [InjectionPoint.THROW]，handler 接收即将抛出的 [Throwable] 并返回替换后的 [Throwable]；
 * 指定 [At.target] 时只匹配直接构造后抛出的同类型异常。返回值会交给原 `ATHROW` 继续抛出，如需跳过抛出应使用 [WrapWithCondition]。
 *
 * 重定向处理器要求：
 *
 * - 方法调用、`invokedynamic` 调用、构造器调用、NEW 构造表达式、字段读取、字段写入、数组元素访问、数组长度、局部变量读取、局部变量待写入值、类型转换、类型判断、条件跳转、switch selector、常量加载与抛异常点重定向支持静态方法、`@JvmStatic` 方法，或 Kotlin `object` 中的实例方法
 ...
 * - 方法调用与 `invokedynamic` 重定向的返回值需与原返回类型兼容；基础类型需精确匹配，引用或数组返回值可为原返回类型的可赋值子类型，也可用 `Any` / `Object` 作为泛型引用返回类型
 ...
 * - 局部变量读取重定向参数为 `xLOAD` 读取出的栈顶表达式值，返回替换值，只替换本次读取结果，不写回槽位
 * - 局部变量写入重定向参数为 `xSTORE` 消费前的待写入栈顶值，返回替换值，返回值交给原 `xSTORE` 继续写入槽位
 * - 类型转换重定向参数为原待转换对象，返回目标类型兼容对象，后续参数可按顺序接收目标方法的部分参数
 * - 类型判断重定向参数为原被判断对象，返回 `Boolean`，后续参数可按顺序接收目标方法的部分参数
 * - 条件跳转重定向参数为原始分支结果 `Boolean`，返回新的分支结果，后续参数可按顺序接收目标方法的部分参数
 * - switch selector 重定向参数为原始 `Int` selector，返回新的 `Int` selector，后续参数可按顺序接收目标方法的部分参数
 * - 常量加载重定向参数为原常量值，返回替换值，后续参数可按顺序接收目标方法的部分参数
 * - 抛异常点重定向参数为即将抛出的 [Throwable]，返回替换后的 [Throwable]，后续参数可按顺序接收目标方法的部分参数
 * - [method] 为空时会按 handler 名称、重定向点和 handler 签名兼容规则匹配唯一同名目标方法；多个兼容重载需要显式指定 [method]
 ...
 * @param target 目标调用、动态调用、构造器、字段、构造类型、类型签名、跳转操作码、常量文本或直接构造异常类型组件；会与 [At.target] 组合构建最终的匹配签名，[InjectionPoint.LOAD] / [InjectionPoint.STORE] / [InjectionPoint.SWITCH] / [InjectionPoint.ARRAY_LENGTH] 不使用该参数
 ...
 * 父类型继承链中可加载的父类/接口，转换后会生成创建目标类实例的静态工厂方法。
 *
 * ## 使用边界
 *
 * - 目标类自身、可加载父类或可加载接口中均找不到目标方法时转换失败。
 * - 普通调用器的参数与返回值必须与目标方法描述符一致。
 * - 普通调用器的静态性必须与目标方法一致。
 * - 接口 `private` 或 `static` 方法不会作为继承默认方法匹配。
 * - 接口私有方法可被普通调用器桥接，调用点会保持 JVM 要求的 special 调用语义。
 * - 构造器调用器只接受目标类、`Object`，或目标类声明父类型继承链中可加载的父类/接口作为返回类型。
 * - 生成的方法若与目标类已有同名同描述符方法冲突，转换会失败。
 *
