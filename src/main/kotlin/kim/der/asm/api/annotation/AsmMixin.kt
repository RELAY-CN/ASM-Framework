 ...
 * - 被修改参数或目标方法参数为对象/数组类型时，对应 handler 参数可声明为 `Any` / `Object`；返回类型对基础类型仍需精确匹配，对象/数组类型可返回可赋值给原参数类型的子类型
 ...
 * - 显式指定 [index] 时，对象/数组变量或目标方法参数对应的 handler 参数可声明为 `Any` / `Object`；返回类型对基础类型仍需精确匹配，对象/数组类型可返回可赋值给原变量类型的子类型
