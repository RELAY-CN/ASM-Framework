 ...
 * 包裹原始操作注解。
 *
 * 用于把目标方法内匹配的方法调用、字段读取或字段写入替换为 handler 调用（语义参考 Mixin Extras 的
 * `@WrapOperation`）。handler 会接收原操作的 receiver（实例调用、实例字段读取与实例字段写入）、
 * 原调用参数或字段写入值与 [Operation]，可选择调用、跳过或多次调用原始操作。
 *
 * 当前实现支持 [InjectionPoint.INVOKE] 方法调用、[InjectionPoint.FIELD] 字段读取与
 * [InjectionPoint.FIELD_ASSIGN] 字段写入。构造器调用和数组操作尚未实现。
 *
 * ASM 方法要求：
 *
 ...
 * - 下一参数必须是 [Operation]，用于执行原始调用、字段读取或字段写入
 ...
 *   并通过 [At.target] 指定要匹配的方法调用、字段读取或字段写入
 *
 * @param method 目标方法签名
 ...
 * 重定向方法调用或字段访问注解。
 *
 * 用于将目标方法中的某个方法调用、字段读取、字段写入或简单数组元素访问重定向到当前 ASM 方法（语义参考 Mixin 的 `@Redirect`）。
 * 当前实现会在字节码中查找匹配的调用指令、字段访问指令或数组元素访问指令，并用重定向处理器调用替换原指令。
 *
 * 字段读取重定向通过 [At.value] 指定 [InjectionPoint.FIELD]，并通过 [At.target] 指定
 * `owner.field:desc`、`field:desc` 或 `field`；字段写入重定向通过 [At.value] 指定
 * [InjectionPoint.FIELD_ASSIGN]，目标格式相同。数组元素访问重定向通过 [At.value] 指定 [InjectionPoint.FIELD]，
 * [At.target] 指定产生数组引用的字段，并通过 [At.args] 中的 `array=get` 或 `array=set` 区分读取与写入。
 *
 * 重定向处理器要求：
 *
 * - 方法调用、字段读取、字段写入与数组元素访问重定向支持静态方法、`@JvmStatic` 方法，或 Kotlin `object` 中的实例方法
 ...
 *
 * @param method 目标方法签名
 * @param target 目标调用或字段签名组件；会与 [At.target] 组合构建最终的匹配签名
 * @param at 调用点信息；[At.value] 决定重定向方法调用、字段读取、字段写入还是数组元素访问，[At.target] 用于指定匹配签名
 * @param ordinal 匹配点序号；`-1` 表示重定向全部匹配点，当前在方法调用、字段读取、字段写入与数组元素访问中生效
 * @param slice 预留参数，当前实现未使用
 * @param remap 是否启用重映射（当前实现未启用，字段仅作为元数据保留）
 * @author Dr (dr@der.kim)
 * @date 2025-11-24
