...
/**
 * 重定向方法调用、构造器调用、字段访问或类型判断注解。
 *
 * 用于将目标方法中的某个方法调用、构造器调用、字段读取、字段写入、简单数组元素访问、数组长度读取或 `INSTANCEOF`
 * 类型判断重定向到当前 ASM 方法（语义参考 Mixin 的 `@Redirect`）。
 * 当前实现会在字节码中查找匹配的调用指令、构造器调用指令、字段访问指令、数组元素访问指令、数组长度指令或
 * 类型判断指令，并用重定向处理器调用替换原指令。
 *
 ...
 * 重定向处理器要求：
 *
 * - 方法调用、构造器调用、字段读取、字段写入、数组元素访问、数组长度与类型判断重定向支持静态方法、`@JvmStatic` 方法，或 Kotlin `object` 中的实例方法
 ...
 * @param at 调用点信息；[At.value] 决定重定向方法调用、构造器调用、字段读取、字段写入、数组元素访问、数组长度读取还是类型判断，[At.target] 用于指定匹配签名
 * @param ordinal 匹配点序号；`-1` 表示重定向全部匹配点，当前在方法调用、构造器调用、字段读取、字段写入、数组元素访问、数组长度读取与类型判断中生效
 * @param slice 切片范围；当前方法调用、构造器调用、字段读取、字段写入、数组元素访问、数组长度与类型判断重定向
 * 支持用 [Slice.from] / [Slice.to] 的 [InjectionPoint.INVOKE] 边界缩小查找范围
