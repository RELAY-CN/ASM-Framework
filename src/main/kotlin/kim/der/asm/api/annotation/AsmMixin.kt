 ...
 *
 * 用于将目标方法中的某个方法调用、构造器调用、字段读取、字段写入或简单数组元素访问重定向到当前 ASM 方法（语义参考 Mixin 的 `@Redirect`）。
 * 当前实现会在字节码中查找匹配的调用指令、构造器调用指令、字段访问指令或数组元素访问指令，并用重定向处理器调用替换原指令。
 *
 ...
 * 重定向处理器要求：
 *
 * - 方法调用、构造器调用、字段读取、字段写入与数组元素访问重定向支持静态方法、`@JvmStatic` 方法，或 Kotlin `object` 中的实例方法
 ...
 * @param at 调用点信息；[At.value] 决定重定向方法调用、构造器调用、字段读取、字段写入还是数组元素访问，[At.target] 用于指定匹配签名
 * @param ordinal 匹配点序号；`-1` 表示重定向全部匹配点，当前在方法调用、构造器调用、字段读取、字段写入与数组元素访问中生效
 * @param slice 预留参数，当前实现未使用
 * @param remap 是否启用重映射（当前实现未启用，字段仅作为元数据保留）
 * @author Dr (dr@der.kim)
 * @date 2025-11-24
